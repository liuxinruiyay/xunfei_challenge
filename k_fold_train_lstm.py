...
from train_utils import train_with_kfold_lstm
from mymodel import CustomBERTModel
...
# 读取整个训练数据集csv文件
df_train = pd.read_csv("./processed_data/final_train.csv")
# X = df_train['text']
# Y = df_train['label']
...
for k, (train_index, valid_index) in enumerate(kf.split(df_train)):
    ...
    train_with_kfold_lstm(k, model, optimizer, train_data_loader, valid_data_loader, num_epochs=5, file_path='./pretrain_final', best_valid_acc = 0.0)
